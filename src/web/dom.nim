...
when defined(js):
  ...
  proc len*(x: Node): int = 0
  proc `[]`*(x: Node; idx: int): Element = nil
  proc getElementById*(id: cstring): Element = nil
  ...
  proc getElementById*(d: Document, id: cstring): Element = nil
  proc createElement*(d: Document, identifier: cstring): Element = nil
  proc createElementNS*(d: Document, namespaceURI, qualifiedIdentifier: cstring): Element = nil
  proc createTextNode*(d: Document, identifier: cstring): Node = nil
  proc createComment*(d: Document, data: cstring): Node = nil

  proc setTimeout*(action: proc(); ms: int): TimeOut = nil
  proc clearTimeout*(t: TimeOut) = discard
  proc setInterval*(action: proc(); ms: int): Interval = nil
  ...
  proc removeEventListener*(et: EventTarget; ev: cstring; cb: proc(ev: Event)) = discard
  ...
  proc open*(w: Window, uri, windowname: cstring,
            properties: cstring = nil): Window = nil
  ...
  proc setInterval*(w: Window, code: cstring, pause: int): Interval = nil
  proc setInterval*(w: Window, function: proc (), pause: int): Interval = nil
  proc setTimeout*(w: Window, code: cstring, pause: int): TimeOut = nil
  proc setTimeout*(w: Window, function: proc (), pause: int): Interval = nil
  proc stop*(w: Window) = discard
  proc requestAnimationFrame*(w: Window, function: proc (time: float)): int = 0
  proc cancelAnimationFrame*(w: Window, id: int) = discard
  proc matchMedia*(w: Window, mediaQueryString: cstring): MediaQueryList = nil

  # Node "methods"
  proc appendData*(n: Node, data: cstring) = discard
  proc cloneNode*(n: Node, copyContent: bool): Node = nil
  proc deleteData*(n: Node, start, len: int) = discard
  proc focus*(e: Node) = discard
  proc getAttribute*(n: Node, attr: cstring): cstring = ""
  proc getAttributeNode*(n: Node, attr: cstring): Node = nil
  proc hasAttribute*(n: Node, attr: cstring): bool = false
  proc hasChildNodes*(n: Node): bool = false
  ...
  proc querySelector*(n: Node, selectors: cstring): Element = nil
  proc querySelectorAll*(n: Node, selectors: cstring): seq[Element] = @[]
  proc compareDocumentPosition*(n: Node, otherNode:Node): int = 0
  proc lookupPrefix*(n: Node): cstring = ""
  proc lookupNamespaceURI*(n: Node): cstring = ""
  proc isDefaultNamespace*(n: Node): bool = false
  proc contains*(n: Node): bool = false
  proc isEqualNode*(n: Node): bool = false
  proc isSameNode*(n: Node): bool = false

  since (1, 3):
    proc getRootNode*(n: Node,options: RootNodeOptions): Node = nil

    # DocumentOrShadowRoot
    proc getSelection*(n: DocumentOrShadowRoot): Selection = nil
    proc elementFromPoint*(n: DocumentOrShadowRoot; x, y: float): Element = nil

    # shadow dom
    proc attachShadow*(n: Element): ShadowRoot = nil
    proc assignedNodes*(n: HTMLSlotElement; options: SlotOptions): seq[Node] = @[]
    proc assignedElements*(n: HTMLSlotElement; options: SlotOptions): seq[Element] = @[]

  # Document "methods"
  proc createAttribute*(d: Document, identifier: cstring): Node = nil
  proc getElementsByName*(d: Document, name: cstring): seq[Element] = @[]
  proc getElementsByTagName*(d: Document, name: cstring): seq[Element] = @[]
  proc getElementsByClassName*(d: Document, name: cstring): seq[Element] = @[]
  proc insertNode*(range: Range, node: Node) = discard
  proc getSelection*(d: Document): Selection = nil
  ...
  proc querySelector*(d: Document, selectors: cstring): Element = nil
  proc querySelectorAll*(d: Document, selectors: cstring): seq[Element] = @[]
  ...
  proc checkValidity*(e: FormElement): bool = false
  proc reportValidity*(e: FormElement): bool = false
  ...
  # Navigator "methods"
  proc javaEnabled*(h: Navigator): bool = false
  since (1, 3):
    proc canShare*(self: Navigator; data: cstring): bool = false
    proc sendBeacon*(self: Navigator; url, data: cstring): bool = false
    proc vibrate*(self: Navigator; pattern: cint): bool = false
    proc vibrate*(self: Navigator; pattern: openArray[cint]): bool = false
    proc registerProtocolHandler*(self: Navigator; scheme, url, title: cstring) = discard

  # ClassList "methods"
  proc add*(c: ClassList, class: cstring) = discard
  proc remove*(c: ClassList, class: cstring) = discard
  proc contains*(c: ClassList, class: cstring): bool = false
  ...
  # KeyboardEvent "methods"
  proc getModifierState*(ev: KeyboardEvent, keyArg: cstring): bool = false

  # MouseEvent "methods"
  proc getModifierState*(ev: MouseEvent, keyArg: cstring): bool = false

  # TouchEvent "methods"
  proc identifiedTouch*(list: TouchList): Touch = nil
  proc item*(list: TouchList, i: int): Touch = nil

  # DataTransfer "methods"
  proc clearData*(dt: DataTransfer, format: cstring) = discard
  proc getData*(dt: DataTransfer, format: cstring): cstring = ""
  proc setData*(dt: DataTransfer, format: cstring, data: cstring) = discard
  proc setDragImage*(dt: DataTransfer, img: Element, xOffset: int64, yOffset: int64) = discard

  # DataTransferItem "methods"
  proc getAsFile*(dti: DataTransferItem): File = nil
  ...
  proc hasItem*(s: Storage, key: cstring): bool = false
  ...
  var
    window* {.threadvar.}: Window
    navigator* {.threadvar.}: Navigator
    screen* {.threadvar.}: Screen
    document* {.threadvar.}: Document 

  proc decodeURI*(uri: cstring): cstring = ""
  proc encodeURI*(uri: cstring): cstring = ""

  proc escape*(uri: cstring): cstring = ""
  proc unescape*(uri: cstring): cstring = ""

  proc decodeURIComponent*(uri: cstring): cstring = ""
  proc encodeURIComponent*(uri: cstring): cstring = ""
  proc isFinite*(x: BiggestFloat): bool = false
  proc isNaN*(x: BiggestFloat): bool = false

  proc newEvent*(name: cstring): Event = nil
  ...
  proc getBoundingClientRect*(e: Node): BoundingRect = BoundingRect()
  proc clientHeight*(): int = 0
  proc clientWidth*(): int = 0

  proc inViewport*(el: Node): bool = false

  proc scrollTop*(e: Node): int = 0
  proc `scrollTop=`*(e: Node, value: int) = discard
  proc scrollLeft*(e: Node): int = 0
  proc scrollHeight*(e: Node): int = 0
  proc scrollWidth*(e: Node): int = 0
  proc offsetHeight*(e: Node): int = 0
  proc offsetWidth*(e: Node): int = 0
  proc offsetTop*(e: Node): int = 0
  proc offsetLeft*(e: Node): int = 0

  since (1, 3):
    func newDomParser*(): DomParser = nil
    func parseFromString*(this: DomParser; str: cstring; mimeType: cstring): Document = nil

    proc newDomException*(): DomException = nil
    proc message*(ex: DomException): cstring = ""
    proc name*(ex: DomException): cstring = ""

    proc newFileReader*(): FileReader = nil
    proc error*(f: FileReader): DomException = nil
    proc readyState*(f: FileReader): FileReaderState = nil
    proc resultAsString*(f: FileReader): cstring = ""
    proc abort*(f: FileReader) = discard
    proc readAsBinaryString*(f: FileReader, b: Blob) = discard
    proc readAsDataURL*(f: FileReader, b: Blob) = discard
    proc readAsText*(f: FileReader, b: Blob|File, encoding = cstring"UTF-8") = discard

  since (1, 5):
    proc elementsFromPoint*(n: DocumentOrShadowRoot; x, y: float): seq[Element] = @[]

  since (1, 7):
    proc insertAdjacentText*(self: Node; position, data: cstring) = discard
    proc insertAdjacentElement*(self: Node; position: cstring; element: Node) = discard
    proc insertAdjacentHTML*(self: Node; position, html: cstring) = discard
    proc after*(self: Node; element: Node): Node = nil
    proc before*(self: Node; element: Node): Node = nil
    proc append*(self: Node; element: Node): Node = nil
    proc closest*(self: Node; cssSelector: cstring): Node = nil
    proc hasAttributeNS*(self: Node; namespace, localName: cstring): bool = false
    proc removeAttributeNS*(self: Node; namespace, attributeName: cstring) = discard
    proc hasPointerCapture*(self: Node; pointerId: SomeNumber): bool = false
    proc releasePointerCapture*(self: Node; pointerId: SomeNumber) = discard
    proc requestPointerLock*(self: Node) = discard
    proc replaceChildren*(self: Node; replacements: Node) = discard
    proc replaceWith*(self: Node; replacements: Node) = discard
    proc scrollIntoViewIfNeeded*(self: Node; centerIfNeeded: bool) = discard
    proc setHTML*(self: Node; html: cstring) = discard
    proc toggleAttribute*(self: Node; name: cstring; force = false): bool = false
    proc matches*(self: Node; cssSelector: cstring): bool = false